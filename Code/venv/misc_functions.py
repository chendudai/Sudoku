...
# b = []
# numDel = (quizzes[:6000000] == 0).sum((1, 2))
# q = np.where(numDel == 55)[0].size
# for i in range(45, 59):
#     b.append(np.where(numDel == i)[0][:q])
# #
# # b = np.where(numDel >= 45)[0]
# c = np.hstack(b)
# d = pd.DataFrame.from_dict({'quizzes': a['quizzes'][c], 'solutions': a['solutions'][c]})
# d.to_csv('dataset/9M_kaggle/sudoku_45_58.csv')
# e = pd.read_csv('dataset/9M_kaggle/sudoku_45_58.csv')
...
def getDataLoaders(batch_size, device):
    if device == torch.device("cuda:0"):
        ...
        # a = pd.read_csv('dataset/9M_kaggle/sudoku_45_58.csv')
    ...
    test_loader = DataLoader(test_set, batch_size=4096, shuffle=False, num_workers=0, drop_last=False)
...
def trainNet(net, batch_size, learning_rate, step, patience, maxDelete, deleteType, valCalcFreq, train_loader, val_loader, device):
    ...
    for n_delete, n_epochs in zip([100], [450]):
            ...
            if relEpoch % valCalcFreq == 0:
                ...
                with torch.no_grad():
                    for data in val_loader:
                        ...
                        val_solved_boards = fillBlank_imporved_complexity(net, val_quizzes, device, maxIter=1)
...
def trackBoard(net, test_loader, device):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            if i > 16:
                break
    plotTrackBoard(trackPreds, trackBoards, trackBoardsSolutions, 5)
...
def compareToBacktracking(net, quizzes, solutions, device, maxIterNet=None):
    ...
    with torch.no_grad():
        for i, (quizz, sol) in enumerate(zip(quizzes, solutions)):
            ...
            solved_net = fillBlank_imporved_complexity(net, quizz_to_net, device)
    ...
    net_solved_each_del = np.zeros(np.unique(numDelete).size)
    for i, nDel in enumerate(np.unique(numDelete)):
        ...
        net_solved_each_del[i] = BoardCorrectIdx_net[indsOfRellevantBoards].sum()/indsOfRellevantBoards.size
    ...
    w = 0.4
    plt.bar(np.unique(numDelete), 100 * net_solved_each_del, width=w, label='Neural Net')
    plt.xlabel('Deleted Number of Elements', fontsize=15)
    plt.ylabel('Accuracy [%]', fontsize=15)
    plt.grid()
    plt.legend(fontsize=15)
    # plt.xticks(np.unique(numDelete), np.unique(numDelete))
